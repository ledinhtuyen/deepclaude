...
impl AnthropicClient {
    ...
    pub(crate) fn build_headers(&self, custom_headers: Option<&HashMap<String, String>>) -> Result<HeaderMap> {
        ...
        headers.insert(
            "Authorization",
            format!("Bearer {}", self.api_token).parse().map_err(|e| ApiError::Internal {
    ...
    /// # Returns
    ///
    /// * `Result<ApiResponse>` - The model's response on success
    ///
    /// # Errors
    ///
    ...
    pub async fn chat(
        ...
        let response_text = response.text().await.map_err(|e| ApiError::AnthropicError { 
        ...
        println!("Anthropic API response: {}", response_text);

        serde_json::from_str(&response_text).map_err(|e| ApiError::AnthropicError { 
